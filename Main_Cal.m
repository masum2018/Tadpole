...
toc;
%% Step2: Geometric Correction(Image_rdg)
% [Image_rdg] = GeometricCorrection(Image_rd,Parameters);
% clear Image_rd;
toc;
%% Step3:Flatfield Correction(Image_rdgf)
[Image_rdgf] = FlatFieldCorrection(Image_rd,Image_d,Parameters);
clear Image_d Image_rdg;
toc;
%% Step4:Binning on Image(Image_rdgfb)
[Image_rdgfb] = Binning(Image_rdgf,Parameters);
clear Image_rdgf;
toc
%% Step5:Apply Color Cal (Image_rdgfbc)
[Result] = ApplyColorCalibrationMatrix(Image_rdgfb,Parameters);
clear Image_rdgfb;
...
toc;
%% Future scope: Parallel/GPU processing
%TODO -for faster processing(out of scope in current scenario):
%{
-Run parallel processing in CPU: Processing speed depends on total number
of CPU
-Run expensive processing part in GPU
-Run both simultaneously
%}

% figure,imagesc(FlatfieldImage(:,:,2));colorbar;
